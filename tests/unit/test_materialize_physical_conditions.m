...
function testAnalyticWindowUsesRequestSpecificGridSpacing(testCase)

requests = make_requests();
request = requests(2);

request.condition_id = "analytic_grid_025";
request.target_frequency_range_hz = [200 200];
request.target_grid_spacing_m = ...
    [0.00025 0.00025];
...
condition = reqml.campaigns.materializePhysicalConditions( ...
    ...
    PatchWindowWavelengths=3, ...
    CsGuessMPerS=3, ...
    SelectedPatchCenterIndicesXZ=[101 101], ...
    InterfaceOrientation="x", ...
    PlannerSeed=91);

verifyEqual(testCase, ...
    condition.geometry_control.resolved_window_size_pixels, ...
    181);

verifyEqual(testCase, ...
    condition.geometry_control.grid_spacing_m, ...
    [0.00025 0.00025], ...
    AbsTol=1e-15);
...
requests = repmat(struct( ...
    ...
    "target_grid_spacing_m", [0.25e-3 0.25e-3], ...
    "deficit_score", 1.0), ...
    3, 1);
...
requests(2).discretization_bin = 2;
requests(2).target_grid_spacing_m = ...
    [0.40e-3 0.40e-3];
...
requests(3).discretization_bin = 3;
requests(3).target_grid_spacing_m = ...
    [0.50e-3 0.50e-3];
