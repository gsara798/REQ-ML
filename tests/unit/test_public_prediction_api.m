...
function testPredictionDoesNotRequireGroundTruth(testCase)
[sample_file, model_file] = fixture_files(testCase);

with_truth = reqml.predictSWS( ...
    sample_file, ...
    Model=model_file, ...
    M=2, ...
    StepPixels=12);
...
wavefield_sample = loaded.wavefield_sample;
wavefield_sample = rmfield(wavefield_sample, "truth");

sample_without_truth = fullfile( ...
    fileparts(sample_file), ...
    "wavefield_sample_without_truth.mat");

save(sample_without_truth, "wavefield_sample");

without_truth = reqml.predictSWS( ...
    sample_without_truth, ...
    Model=model_file, ...
    M=2, ...
    StepPixels=12);

verifyFalse(testCase, without_truth.metadata.input_has_truth);
verifyTrue(testCase, with_truth.metadata.input_has_truth);

verifyEqual(testCase, ...
    without_truth.cs_map, ...
    with_truth.cs_map, ...
    AbsTol=1e-12);
...
verifyEqual(testCase, ...
    without_truth.valid_mask, ...
    with_truth.valid_mask);
