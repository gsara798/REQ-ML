...
arguments
    ...
    options.PreIterationDeficitState table = table()
...
if options.UseDirectedCenterSelection || ...
    ...
    if analytic_center_control.enabled && ismember( ...
            options.CenterSelectionMode,[ ...
                "analytic_target_plus_deficit_aware_centers", ...
                "analytic_deficit_aware_final"])
        validate_analytic_center_control( ...
            analytic_center_control, data, win_size, run);
        if isempty(options.PreIterationDeficitState)
            error("reqml:MissingPreIterationDeficitData", ...
                "Hybrid center selection requires pre-iteration deficits.");
        end
        dnom = reqml.coverage.computeNominalAngularCoverage( ...
            double(run.direction_count), double(run.solid_angle_sr), ...
            MaximumDirectionCount=128);
        target = struct( ...
            "center_indices_xz", ...
                analytic_center_control.selected_patch_center_indices_xz, ...
            "requested_cell_key", analytic_center_control.requested_cell_key, ...
            "predicted_discrete_purity", ...
                analytic_center_control.predicted_discrete_purity);
        center_selection_diagnostics = ...
            reqml.datasets.selectDeficitAwareCenters( ...
                data.truth, win_size, options.PreIterationDeficitState, target, ...
                ...
                FrequencyHz=double(run.frequency_hz), ...
                Dnom=dnom, ...
                GridSpacingM=[ ...
                    double(data.dx_m) ...
                    double(data.dz_m)], ...
                ...
                SeparationPolicy=resolve_separation_policy(options), ...
                ...
                SameCellSeparationFraction= ...
                    options.SameCellSeparationFraction, ...
                MinimumValidFraction=options.MinimumValidFraction, ...
                RealizationsPerCondition=options.RealizationsPerCondition, ...
                DeterministicSeed=analytic_center_control.condition_seed + ...
                    options.CenterSelectionSeedOffset, ...
                RunId=string(run.run_id), ConditionId=string(run.condition_id), ...
                XCoordinatesM=data.axes.x_m, ZCoordinatesM=data.axes.z_m, ...
                InterfaceOrientation= ...
                    analytic_center_control.interface_orientation, ...
                InterfacePositionM=analytic_center_control.interface_position_m);
        center_selection = center_selection_diagnostics.centers;
        extraction_arguments(end + 1, :) = {"CenterIndices", ...
            center_selection{:, ["cx","cz"]}};
    elseif analytic_center_control.enabled
        validate_analytic_center_control( ...
...
if isfinite(options.MaximumExamplesPerRun)
    ...
    [extraction.examples,quota]=reqml.datasets.selectPatchQuota( ...
...
if ~isempty(center_selection)
    ...
    summary_record.analytic_target_example_count = sum( ...
        center_selection.center_selection_role == "analytic_target");
    summary_record.opportunistic_example_count = sum( ...
        center_selection.center_selection_role == ...
            "opportunistic_deficit_fill");
    summary_record.helped_cell_count = numel(unique( ...
        center_selection.achieved_cell_key( ...
            center_selection.selected_for_opportunistic_cell)));
    for name = rejection_field_names()
        summary_record.("rejected_" + name) = double( ...
            center_selection_diagnostics.rejection_counts.(name));
    end
end
...
function examples = attach_center_selection_metadata(examples, selection)

[found, location] = ismember( ...
    examples{:, ["cx","cz"]}, selection{:, ["cx","cz"]}, "rows");
if ~all(found)
    error("reqml:SelectedCenterExtractionMismatch", ...
        "Extractor output does not match the selected center plan.");
end
selection = selection(location,:);
selection.center_x_index = selection.cx;
selection.center_z_index = selection.cz;
skip = ["cx","cz","coverage_valid","is_analytic_target","tie_breaker"];
names = setdiff(string(selection.Properties.VariableNames), skip, "stable");
for name = names
    if ismember(name, string(examples.Properties.VariableNames))
        continue
    end
    examples.(name) = selection.(name);
end

end


function policy = resolve_separation_policy(options)
if options.CenterSelectionMode=="analytic_deficit_aware_final"
    policy="relationship_aware";
else
    policy="uniform";
end
end
...
for condition_index = 1:numel(plan.physical_conditions)
    ...
    for sample_index = reshape(matches, 1, [])
        ...
        if isfield(requested, "requested_cell_key")
            controls(sample_index).requested_cell_key = ...
                string(requested.requested_cell_key);
        elseif isfield(requested, "discretization_bin")
            controls(sample_index).requested_cell_key = compose( ...
            ...
            error("reqml:MissingRequestedDiscretizationIdentity", ...
                ["Analytic condition '%s' lacks a persisted " + ...
                 "five-dimensional coverage identity."], ...
                condition_id);
        end
        controls(sample_index).condition_seed = double(condition.seed);
        controls(sample_index).interface_orientation = ...
            string(control.interface_orientation);
        controls(sample_index).interface_position_m = ...
            double(control.interface_position_m);
    end
end
...
record.helped_cell_count = 0;
for name = rejection_field_names()
    record.("rejected_" + name) = 0;
end

end

function names = rejection_field_names()
names = ["patch_outside_domain","insufficient_valid_fraction", ...
    "outside_coverage_grid","cell_already_complete", ...
    "no_remaining_deficit","per_cell_quota_reached", ...
    "per_run_quota_reached","insufficient_spatial_separation", ...
...
config.center_selection_seed_offset = ...
    options.CenterSelectionSeedOffset;
config.center_selection_mode = options.CenterSelectionMode;
config.maximum_centers_per_run = options.MaximumCentersPerRun;
config.maximum_centers_per_cell_per_run = ...
    options.MaximumCentersPerCellPerRun;
config.minimum_center_separation_fraction = ...
    options.MinimumCenterSeparationFraction;
config.analytic_to_opportunistic_separation_fraction = ...
    options.AnalyticToOpportunisticSeparationFraction;
config.different_cell_separation_fraction = ...
    options.DifferentCellSeparationFraction;
config.same_cell_separation_fraction = ...
    options.SameCellSeparationFraction;
config.realizations_per_condition = options.RealizationsPerCondition;
