...
diagnostics=struct("available_patch_count",available, ...
    "selected_patch_count",selected_count,"target_patch_count", ...
    double(target_count),"deficit_patch_count", ...
    max(0,double(target_count)-selected_count), ...
    "selection_seed",double(selection_seed), ...
    "selected_candidate_indices",indices, ...
    "complete",selected_count>=target_count, ...
    "subsampled",selected_count<available);
end
