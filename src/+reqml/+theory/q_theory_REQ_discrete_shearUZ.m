...
%   - 'PartialDiffuse3D'
%
% PartialDiffuse3D is defined operationally as:
%   q_partial = 0.5 * (q_single_wave + q_diffuse_3d)
%
% Notes
% -----
...
field_type = string(P.FieldType);

if ~any(strcmpi(field_type, valid_field_types))
    error("reqml:InvalidTheoryFieldType", ...
        "Unsupported FieldType '%s'.", field_type);
end

if strcmpi(field_type, "PartialDiffuse3D")
    common_arguments = { ...
        'M', P.M, ...
        'Gamma', P.Gamma, ...
        ...
        'CraterPower', P.CraterPower, ...
        'TheoryMode', P.TheoryMode, ...
        'Plot', false, ...
        'BuildTransferMatrix', P.BuildTransferMatrix, ...
        'UseDonutFilter', P.UseDonutFilter, ...
        'DonutCsMin', P.DonutCsMin, ...
        'DonutCsMax', P.DonutCsMax, ...
        'DonutTaperRel', P.DonutTaperRel};
    ...
    out.valid = ...
        single_wave.valid && ...
        diffuse_3d.valid && ...
        isfinite(out.q_theory_discrete) && ...
        out.q_theory_discrete >= 0 && ...
        out.q_theory_discrete <= 1;

    out.components = struct();
    out.components.single_wave = single_wave;
    out.components.diffuse_3d = diffuse_3d;
...
if (ischar(P.Nbins) || isstring(P.Nbins)) && ...
        strcmpi(string(P.Nbins), "auto")
    oversample_factor = 1;
    Nbins = round(kmax / (dk_rad / oversample_factor));
    Nbins = max(16, Nbins);
elseif isnumeric(P.Nbins) && isscalar(P.Nbins) && ...
        isfinite(P.Nbins) && P.Nbins >= 1
    Nbins = max(16, round(P.Nbins));
else
    error("reqml:InvalidTheoryNbins", ...
        "Nbins must be a positive numeric scalar or 'auto'.");
end
...
out.field_type = string(P.FieldType);
out.q_theory_discrete = out.q_th;
out.q_single_wave_discrete = NaN;
out.q_diffuse3d_discrete = NaN;

if strcmpi(out.field_type, "SingleWave")
    out.q_single_wave_discrete = out.q_th;
elseif strcmpi(out.field_type, "Diffuse3D")
    out.q_diffuse3d_discrete = out.q_th;
end
