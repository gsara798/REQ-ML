classdef Test12Analysis
    ...
    methods (Static)
        ...
        function T = prepareFeatureTable(T)
            T = reqml.analysis.Test12Analysis.addLocalEcumFeatures(T);
            if ~ismember('row_key', string(T.Properties.VariableNames))
                T.row_key = reqml.analysis.Test12Analysis.makeRowKey(T);
            end
        end
        ...
        function specs = buildModelSpecs(T)
            ...
            local = reqml.analysis.Test12Analysis.existingPredictors(T, base_local);
            global_predictors = reqml.analysis.Test12Analysis.existingPredictors(T, base_global);
            ...
            for i = 1:numel(families)
                for j = 1:numel(variants)
                    ...
                    predictors = reqml.analysis.Test12Analysis.existingPredictors( ...
                        T, unique([families(i).predictors; add(:)], 'stable'));

                    if role == "operational"
                        predictors = reqml.analysis.Test12Analysis.filterOperationalPredictors(predictors);
                        reqml.analysis.Test12Analysis.assertNoLeakage(predictors, families(i).name);
                    end
        ...
        function assertGroupedSplit(T, train_mask, test_mask, heldout_var, heldout_value)
            ...
            assert(~any(reqml.analysis.Test12Analysis.isGroupValue( ...
                x(train_mask), heldout_value)), ...
                'Heldout value appears in train split for %s = %s.', ...
                heldout_var, reqml.analysis.Test12Analysis.valueToString(heldout_value));
            assert(all(reqml.analysis.Test12Analysis.isGroupValue( ...
        ...
        function T_sws = addSwsMetrics(T_pred, T_ref)
            ...
            T_sws.cs_pred = reqml.analysis.Test12Analysis.qToCs( ...
        ...
        function cs = qToCs(q, mappings, f0)
            ...
            for i = 1:numel(q)
                ...
                k = reqml.quantile.quantile_to_k(mappings{i}, q(i));
                cs(i) = 2*pi*f0(i) ./ k;
            end
        end
        ...
        function T_delta = deltaVsNoCsGuess(T_metrics, group_vars)
            ...
            for i = 1:height(keys)
                ...
                for k = 1:numel(variants)
                    ...
                    T_delta = reqml.analysis.Test12Analysis.concatTables(T_delta, row);
                end
            end
        end
        ...
        function T = concatTables(A, B)
            ...
            A = reqml.analysis.Test12Analysis.addMissingColumns(A, vars_all);
            B = reqml.analysis.Test12Analysis.addMissingColumns(B, vars_all);
        ...
        function key = makeRowKey(T)
            ...
            reqml.analysis.Test12Analysis.requireVars(T, required, 'row_key');
        ...
        function assertNoLeakage(predictors, model_name)
            filtered = reqml.analysis.Test12Analysis.filterOperationalPredictors(predictors);
    ...
    methods (Static, Access = private)
        function T = addLocalEcumFeatures(T)
            ...
            feat0 = reqml.quantile.extract_ecum_shape_features(T.req_mapping{idx});
            ...
            for i = 1:height(T)
                ...
                feat_i = reqml.quantile.extract_ecum_shape_features(T.req_mapping{i});
                for j = 1:numel(names)
