...
%
%   OUT = reqml.estimators.req_estimator_map(Uxz, cfg, feat_cfg)
%
% Required inputs
...
p = inputParser;
p.FunctionName = 'reqml.estimators.req_estimator_map';
...
elseif quantile_mode == "theory_discrete"
    theory_out = reqml.theory.q_theory_REQ_discrete_shearUZ( ...
...
[q_local_i, req_curve_i] = reqml.quantile.compute_quantile_from_patch( ...
    patch, cfg, req_cfg);
...
k_i = reqml.quantile.quantile_to_k(req_curve_i, q_i);
cs_i = reqml.quantile.quantile_to_cs(req_curve_i, q_i, cfg.f0);
valid_i = isfinite(q_i) && isfinite(cs_i);
mapping_i = reqml.quantile.make_req_mapping(req_curve_i);
...
shape_i = reqml.quantile.extract_ecum_shape_features(req_curve_i);

if compute_features
    if reuse_req_spectrum_for_features
        feat_i = reqml.features.req_extract_patch_features_from_curve( ...
            req_curve_i, feat_cfg);
    else
        feat_i = reqml.features.req_extract_patch_features( ...
