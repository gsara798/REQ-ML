...
%
% Usage
%   [T_raw, sweep] = reqml.studies.run_aperture_sweep(cfg, feat_cfg);
%
%   [T_raw, sweep] = reqml.studies.run_aperture_sweep(RUN);
%
% Output
%   T_raw:
...
p = inputParser;
p.FunctionName = 'reqml.studies.run_aperture_sweep';
...
schedule = reqml.simulate.build_aperture_schedule( ...
    sampling_mode, ...
    num_steps);
...
for sidx = 1:numel(step_indices)
    ...
    cfg_step = reqml.simulate.apply_sampling_step( ...
        cfg, ...
        sampling_mode, ...
        step_value);
    ...
    for r = 1:num_realizations
        ...
        [req_cfg_i, feat_cfg_i] = reqml.config.default_req_config( ...
            cfg_i, ...
            feat_cfg, ...
            opt.ReqOptions{:});

        sim = reqml.simulate.run_single_simulation(cfg_i);
        ...
        if compute_global_req
            [global_req.q_theory, global_req.curve, global_req.features] = ...
                reqml.quantile.compute_global_quantile_from_field( ...
                    sim.Uxz, cfg_i, req_cfg_i, feat_cfg_i);

            global_req.shape_features = ...
                reqml.quantile.extract_ecum_shape_features( ...
                    global_req.curve);

            global_req.mapping = reqml.quantile.make_req_mapping( ...
                global_req.curve);
        end

        patch_pack = reqml.simulate.build_patch_windows( ...
            cfg_i, ...
            feat_cfg_i, ...
        ...
        for pidx = 1:patch_pack.n_patches
            ...
            feat = reqml.features.req_extract_patch_features( ...
                V_patch, ...
                cfg_i.dx, ...
                cfg_i.dz, ...
                cfg_i.f0, ...
            ...
            [q_theory, req_curve] = reqml.quantile.compute_quantile_from_patch( ...
                V_patch, ...
                cfg_i, ...
                req_cfg_i);

            req_shape_features = ...
                reqml.quantile.extract_ecum_shape_features(req_curve);
            ...
            k_req_q_theory = reqml.quantile.quantile_to_k( ...
                req_curve, ...
                q_theory);

            cs_req_q_theory = reqml.quantile.quantile_to_cs( ...
                req_curve, ...
                q_theory, ...
                cfg_i.f0);
            ...
            if store_req_mapping
                rows(row_idx).req_mapping = { ...
                    reqml.quantile.make_req_mapping(req_curve)};
            end
        ...
        if (plot_step_diagnostics || save_step_diagnostics) && rep_diag_found
            ...
            figs_diag = reqml.figures.plot_step_diagnostics( ...
                sim, ...
                patch_pack, ...
                rep_diag, ...
                'StepIndex', step_idx, ...
            ...
            if save_step_diagnostics
                ...
                reqml.figures.save_figure_bundle( ...
                    figs_diag, ...
                    step_diagnostic_dir, ...
                    base_name, ...
