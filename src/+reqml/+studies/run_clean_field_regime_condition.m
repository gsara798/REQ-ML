...
p = inputParser;
p.FunctionName = 'reqml.studies.run_clean_field_regime_condition';
...
for realization_idx = 1:CFG.EXP.num_realizations
    ...
    sim = reqml.simulate.run_single_simulation(cfg);
...
cfg = reqml.config.default_sim_config( ...
    'cs_bg', cs_bg, ...
...
[req_cfg, feat_cfg] = reqml.config.default_req_config( ...
...
global_shape = reqml.quantile.extract_ecum_shape_features(global_curve);
global_mapping = reqml.quantile.make_req_mapping(global_curve);
...
for patch_idx = 1:patch_pack.n_patches
    ...
    [q_theory, req_curve] = reqml.quantile.compute_quantile_from_patch( ...
        patch, cfg, req_cfg);
    local_shape = reqml.quantile.extract_ecum_shape_features(req_curve);
    ...
    row.k_req_q_theory = reqml.quantile.quantile_to_k(req_curve, q_theory);
    row.cs_req_q_theory = reqml.quantile.quantile_to_cs( ...
        req_curve, q_theory, cfg.f0);
    row.req_mapping = {reqml.quantile.make_req_mapping(req_curve)};
