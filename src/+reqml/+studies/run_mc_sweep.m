...
%RUN_MC_SWEEP Run a Monte Carlo parameter sweep for reqml.
%
...
% Usage:
%
%   [T_mc, MC] = reqml.studies.run_mc_sweep( ...
%       'test_04_mc_baseline', ...
%       'RootDir', root_dir);
%
%   [T_mc, MC] = reqml.studies.run_mc_sweep( ...
...
p = inputParser;
p.FunctionName = 'reqml.studies.run_mc_sweep';
...
if isstruct(CFG_or_profile)
    ...
    if isempty(config_dir)
        CFG = reqml.config.load_profile_config( ...
            profile_name, ...
            'RootDir', root_dir);
    else
        CFG = reqml.config.load_profile_config( ...
...
[T_axes, T_conditions, CFG_base] = ...
    reqml.config.build_sweep_preview(CFG);
...
for pos = 1:n_to_run
    ...
    try
        ...
        RUN_i = reqml.config.resolve_run_config( ...
            CFG_i, ...
            'RootDir', root_dir, ...
        ...
        if save_condition_summary_figures
            ...
            reqml.figures.plot_run_summary( ...
                T_i, sweep_i, RUN_i);
...
[T_i, sweep_i] = reqml.studies.run_aperture_sweep( ...
