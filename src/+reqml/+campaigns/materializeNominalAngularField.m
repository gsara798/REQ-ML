...
% With MaximumSolidAngleSr=4*pi this reduces to:
%   minimum_count = ceil(target_dnom * Nmax).
...
% Numerical protection only. A meaningful violation is rejected below.
solid_angle_sr = min( ...
    options.MaximumSolidAngleSr, ...
    max(options.MinimumSolidAngleSr, solid_angle_sr));
