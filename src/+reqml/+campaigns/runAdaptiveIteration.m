...
pre_iteration_deficit_state = resolve_pre_iteration_deficit_state(options);
pre_iteration_deficits_csv = "";
if ~isempty(pre_iteration_deficit_state)
    pre_iteration_deficits_csv = fullfile( ...
        output_directory, "pre_iteration_deficit_state.csv");
    writetable(pre_iteration_deficit_state, pre_iteration_deficits_csv);
end
...
dataset = reqml.datasets.build_dataset_from_campaign( ...
    ...
    PreIterationDeficitState=pre_iteration_deficit_state, ...
...
result.paths.pre_iteration_deficit_state = ...
    string(pre_iteration_deficits_csv);

end


function state = resolve_pre_iteration_deficit_state(options)

if ~ismember(options.CenterSelectionMode,[ ...
...
if strlength(options.PreviousCoverageReportFile) > 0
    if ~isfile(options.PreviousCoverageReportFile)
        error("reqml:MissingPreIterationCoverageReport", ...
            "Previous coverage report was not found: %s", ...
            options.PreviousCoverageReportFile);
    end
    loaded = load(options.PreviousCoverageReportFile, "coverage_report");
    if ~isfield(loaded,"coverage_report") || ...
            ~isfield(loaded.coverage_report,"summary")
        error("reqml:InvalidPreIterationCoverageReport", ...
            "Previous coverage report lacks a complete summary.");
    end
    summary = loaded.coverage_report.summary;
elseif strlength(options.PreviousDatasetDirectory) == 0
    empty = table(zeros(0,1),zeros(0,1),zeros(0,1),zeros(0,1), ...
        ...
        "example_count","independent_run_count", ...
        "independent_condition_count"]);
    summary = reqml.coverage.completeCoverageGrid(empty, ...
        options.SwsEdges, options.FrequencyEdges, ...
        options.PurityEdges, options.DiffusivityEdges);
else
    error("reqml:MissingPreIterationDeficitData", ...
        "Hybrid incremental extraction requires the previous coverage report.");
end

state = reqml.coverage.findCoverageDeficits(summary, ...
    MinimumExamples=options.MinimumExamples, ...
    MinimumIndependentRuns=options.MinimumIndependentRuns, ...
    MinimumIndependentConditions=options.MinimumIndependentConditions, ...
    MinimumGeometrySeeds=options.MinimumGeometrySeeds, ...
