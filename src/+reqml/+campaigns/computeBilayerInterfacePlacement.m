...
% and REQ-ML modal material fraction. The closest candidate inside the
% requested half-open purity interval is selected. A degenerate range
% [1,1] requests an exactly pure patch and requires the interface to lie at
% least one grid interval outside the patch support.
...
candidate_positions_m = (candidate_indices - 1) * axis_spacing_m;
...
if ~any(feasible)
    error("reqml:AnalyticBilayerNoDiscreteOffset", ...
        ...
        "[%g,%g) for the requested patch support.", ...
...
center_position_m = ...
    (center_axis_index - 1) * axis_spacing_m;
...
placement.selected_patch_center_m = ...
    (selected_patch_center_indices_xz - 1) .* grid_spacing_m;
...
placement.diagnostics.coordinate_convention = ...
    "one_based_indices; coordinate_m=(index-1)*spacing_m; positive_side_is_strict";
