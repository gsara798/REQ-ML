...
for index=1:numel(M_values)
    ...
    if index==1, reference=current; end
end
...
dataset=struct("schema_name","reqml_homogeneous_dual_M_dataset", ...
    ...
    "campaign_runs_csv",string(campaign_csv),"sample_count",reference.sample_count, ...
    "analysis_sample_count",height(sample_summaries),"example_count",height(examples), ...
    "inventory",reference.inventory,"sample_summaries",sample_summaries, ...
