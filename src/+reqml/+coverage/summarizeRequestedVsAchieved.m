...
geometry_control_mode = repmat("legacy", condition_count, 1);

requested_purity_bin = zeros(condition_count, 1);
requested_diffusivity_bin = zeros(condition_count, 1);
requested_sws_bin = zeros(condition_count, 1);
requested_frequency_bin = zeros(condition_count, 1);
...
target_purity = NaN(condition_count, 1);
predicted_discrete_purity = NaN(condition_count, 1);
interface_offset_pixels = NaN(condition_count, 1);
interface_offset_m = NaN(condition_count, 1);
interface_position_m = NaN(condition_count, 1);
selected_patch_center_x_index = NaN(condition_count, 1);
selected_patch_center_z_index = NaN(condition_count, 1);
selected_patch_center_x_m = NaN(condition_count, 1);
selected_patch_center_z_m = NaN(condition_count, 1);
dominant_material_side = strings(condition_count, 1);
...
predicted_purity_observation = NaN(condition_count, 1);
...
for index = 1:condition_count
    ...
    if isfield(condition, "geometry_control")
        ...
        if isfield(control, "geometry_control_mode")
            geometry_control_mode(index) = ...
                string(control.geometry_control_mode);
        end

        target_purity(index) = resolve_control_scalar( ...
            control, "target_purity");
        predicted_discrete_purity(index) = ...
            resolve_control_scalar( ...
                control, "predicted_discrete_purity");
        interface_offset_pixels(index) = ...
            resolve_control_scalar( ...
                control, "interface_offset_pixels");
        interface_offset_m(index) = resolve_control_scalar( ...
            control, "interface_offset_m");
        interface_position_m(index) = resolve_control_scalar( ...
            control, "interface_position_m");

        center_indices = resolve_control_pair( ...
            control, "selected_patch_center_indices_xz");
        center_m = resolve_control_pair( ...
            control, "selected_patch_center_m");

        selected_patch_center_x_index(index) = center_indices(1);
        selected_patch_center_z_index(index) = center_indices(2);
        selected_patch_center_x_m(index) = center_m(1);
        selected_patch_center_z_m(index) = center_m(2);

        if isfield(control, "dominant_material_side")
            dominant_material_side(index) = ...
                string(control.dominant_material_side);
        end
    end
    ...
    requested_sws_bin(index) = double(requested.sws_bin);
    requested_frequency_bin(index) = ...
    ...
    sws_bins = double( ...
        assigned_examples.coverage_sws_bin(valid_mask));

    frequency_bins = double( ...
        assigned_examples.coverage_frequency_bin(valid_mask));

    request_evaluation_mask = valid_mask;
    if ismember("center_selection_role", ...
            string(assigned_examples.Properties.VariableNames))
        request_evaluation_mask = request_evaluation_mask & ...
            string(assigned_examples.center_selection_role) == ...
                "analytic_target";
    end

    request_sws_bins = double(assigned_examples. ...
        coverage_sws_bin(request_evaluation_mask));
    request_frequency_bins = double(assigned_examples. ...
        coverage_frequency_bin(request_evaluation_mask));
    request_purity_bins = double(assigned_examples. ...
        coverage_purity_bin(request_evaluation_mask));
    request_diffusivity_bins = double(assigned_examples. ...
        coverage_diffusivity_bin(request_evaluation_mask));

    requested_mask = ...
        request_sws_bins == requested_sws_bin(index) & ...
        request_frequency_bins == requested_frequency_bin(index) & ...
        request_purity_bins == requested_purity_bin(index) & ...
        request_diffusivity_bins == requested_diffusivity_bin(index);
    ...
    achieved_cells(index) = ...
        join_coverage_cells( ...
            sws_bins, frequency_bins, ...
            purity_bins, diffusivity_bins);
    ...
    prediction_values = double( ...
        assigned_examples.truth_material_purity(request_evaluation_mask));
    if ~isempty(prediction_values)
        predicted_purity_observation(index) = median(prediction_values);
    end
...
predicted_vs_achieved_purity_error = ...
    predicted_purity_observation - predicted_discrete_purity;
...
summary = table( ...
    ...
    selected_patch_center_x_index, ...
    selected_patch_center_z_index, ...
    selected_patch_center_x_m, ...
    selected_patch_center_z_m, ...
...
function value = resolve_control_scalar(control, field_name)

value = NaN;

if isfield(control, field_name)
    candidate = double(control.(field_name));
...
function value = resolve_control_pair(control, field_name)
...
function text = join_coverage_cells( ...
        sws_bins, frequency_bins, purity_bins, diffusivity_bins)

pairs = unique( ...
    [double(sws_bins(:)), ...
     double(frequency_bins(:)), ...
     double(purity_bins(:)), ...
     double(diffusivity_bins(:))], ...
    "rows");
