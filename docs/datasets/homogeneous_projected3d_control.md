...
The current dataset preserves the same 300 wavefields and REQ extraction configuration used in the previous control build, while adding a traceable mapping between projected-3D direction count and the theoretical REQ field class used for comparison.
...
The current dataset adds the following nontrainable diagnostic columns:
