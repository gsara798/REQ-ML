...
%
% Usage:
%   figs = reqml.figures.plot_step_diagnostics(sim, patch_pack, rep);
%
% This function is intended to be called from:
%
%   reqml.studies.run_aperture_sweep
%
% Inputs:
%   sim:
%       Output from reqml.simulate.run_single_simulation.
%
%   patch_pack:
%       Output from reqml.simulate.build_patch_windows.
%
%   rep:
%       Representative REQ diagnostic structure, typically req_curve from
%       reqml.quantile.compute_quantile_from_patch.
...
reqml.figures.apply_style(figs.field, ax1);
reqml.figures.format_axis_ticks(ax1, nTickDecimals);

if logical(opt.NormalizeField) && ~strcmpi(opt.FieldComponent, 'abs')
    cb1.Ticks = [-1 0 1];
end

reqml.figures.format_colorbar_ticks(cb1, nTickDecimals);
...
if has_spectrum
    ...
    reqml.figures.apply_style(figs.spectrum, ax2);
    reqml.figures.format_axis_ticks(ax2, nTickDecimals);
    reqml.figures.format_colorbar_ticks(cb2, nTickDecimals);
...
if has_radial
    ...
    reqml.figures.apply_style(figs.radial, ax3);
    ...
    reqml.figures.format_axis_ticks(ax3, nTickDecimals);
