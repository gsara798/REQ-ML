% Circular inclusion wavefield -> current REQ Q0 SWS map.
cs_background = 2.0;
cs_inclusion = 3.0;
inclusion_radius_mm = 15;
f0 = 400;
...
campaign_file = fullfile(output_root, "inclusion_campaign.json");
campaign = reqml.examples.buildSimulationCampaign("inclusion", ...
    OutputDirectory=output_root, CsBackground=cs_background, ...
    CsInclusion=cs_inclusion, InclusionRadiusMm=inclusion_radius_mm, ...
    FrequencyHz=f0, FieldRegime=field_regime);
...
title(sprintf("REQ Q0 inclusion SWS, M=%d", M));
