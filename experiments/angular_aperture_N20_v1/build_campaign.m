...
writetable(conditions, ...
    fullfile(campaign_dir,"physical_conditions.csv"));

writetable(runs, ...
    fullfile(campaign_dir,"planned_runs.csv"));

manifest = struct();
manifest.schema_name = ...
    "reqml_angular_aperture_N20_campaign_manifest";
...
manifest.aperture_fractions = ...
    double(cfg.design.solid_angle_fraction(:));
...
write_json( ...
    fullfile(campaign_dir,"campaign_manifest.json"), ...
    manifest);

fprintf("\nN=20 campaign materialization PASSED\n");
fprintf("===================================\n");
fprintf("Conditions      : %d\n",height(conditions));
fprintf("Runs            : %d\n",height(runs));
fprintf("Directions      : %d\n",cfg.design.direction_count);
fprintf("SWS values      : %s\n", ...
