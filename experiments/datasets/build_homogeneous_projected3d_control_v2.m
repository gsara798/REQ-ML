%% BUILD_HOMOGENEOUS_PROJECTED3D_CONTROL_V2
...
%
% Version 2 adds explicit trainable spatial and spectral discretization
% context:
%
%   GRID_dx_m
%   GRID_dz_m
...
%   REQ_Nbins_effective
%
% This remains a control dataset, not the final multi-M / multi-cs_guess
% training dataset.
...
dataset_id = "homogeneous_projected3d_control_v2";
...
for variable_name = required_context.'
    row = registry( ...
    ...
    assert(row.role == "metadata");
    assert(row.group == "operational_context");
end

assert(all(isfinite( ...
    dataset.examples.REQ_Nbins_effective)));

assert(all(isfinite( ...
    dataset.examples.REQ_dkx_rad_m)));

assert(all(isfinite( ...
    dataset.examples.REQ_dkz_rad_m)));
