%% run_stage_c2_projected_diffuse3d_probe.m
% Stage C2: projected-diffuse 3D Eikonal probe.
%
...
%
% Runtime controls:
%   ADAPTIVE_REQ_EIKONAL_STAGE_C2_MODE = validate_only | quick | full
%   ADAPTIVE_REQ_EIKONAL_STAGE_C2_FORCE_REBUILD = true | false
%   ADAPTIVE_REQ_EIKONAL_STAGE_C2_USE_SOURCE_PARFOR = true | false
%   ADAPTIVE_REQ_EIKONAL_STAGE_C2_USE_WINDOW_PARFOR = true | false
...
CFG.Mode = lower(strtrim(string(getenv('ADAPTIVE_REQ_EIKONAL_STAGE_C2_MODE'))));
if CFG.Mode == "", CFG.Mode = "quick"; end
assert(ismember(CFG.Mode, ["validate_only","quick","full"]), ...
    'ADAPTIVE_REQ_EIKONAL_STAGE_C2_MODE must be validate_only, quick, or full.');
CFG.ForceRebuild = env_true('ADAPTIVE_REQ_EIKONAL_STAGE_C2_FORCE_REBUILD', false);
CFG.UseSourceParfor = env_true('ADAPTIVE_REQ_EIKONAL_STAGE_C2_USE_SOURCE_PARFOR', true);
CFG.UseWindowParfor = env_true('ADAPTIVE_REQ_EIKONAL_STAGE_C2_USE_WINDOW_PARFOR', true);
CFG.UseParallel = CFG.UseSourceParfor || CFG.UseWindowParfor;
CFG.CacheVersion = 2;

OUT = make_output_dirs(root_dir, CFG);
write_json_copy(CFG, fullfile(OUT.root_dir, 'stage_C2_projected_diffuse3d_probe_effective_config.json'));
...
if isfield(CFG.Simulation, 'PhaseModel') && lower(string(CFG.Simulation.PhaseModel)) == "eikonal3d_projected"
    assert(exist(fullfile(waveSrc, '+simcore', '+eikonal', 'fromPointSource3DExtruded.m'), 'file') == 2, ...
        'Stage C2 requested eikonal3D_projected, but fromPointSource3DExtruded.m was not found.');
end
...
fprintf('Phase model: %s\n', string(CFG.Simulation.PhaseModel));
...
for ci = 1:numel(CONDS)
    ...
    cacheFile = fullfile(OUT.cache_dir, "stageC2__" + sanitize(C.condition_id) + ".mat");
    tCondition = tic;
    try
        ...
        if ~CFG.ForceRebuild && exist(cacheFile, 'file') == 2
            ...
            if isfield(S, 'cond_signature') && isequaln(S.cond_signature, sig)
                ...
                if isfield(S, 'T_ang')
                    angParts{end+1,1} = S.T_ang; %#ok<SAGROW>
                end
        ...
        T_ang = projected_angular_diagnostics(sim, T_condition, C, CFG); %#ok<NASGU>
        MAPS = compact_maps_for_cache(sim, T_condition, C, CFG); %#ok<NASGU>
        save(cacheFile, 'T_condition', 'T_sources', 'T_ang', 'cond_signature', 'MAPS', 'simDiag', 'reqDiag', '-v7.3');

        if should_save_representative_map(C, CFG)
            plot_representative_maps(sim, T_condition, C, CFG, OUT);
            mapCount = mapCount + 1;
        end
        ...
        patchParts{end+1,1} = T_condition; %#ok<SAGROW>
        sourceParts{end+1,1} = T_sources; %#ok<SAGROW>
        angParts{end+1,1} = T_ang; %#ok<SAGROW>
        logParts{end+1,1} = condition_log_row(C, "completed", height(T_condition), elapsed, ""); %#ok<SAGROW>
    catch ME
        elapsed = toc(tCondition);
        warning('StageC2:ConditionFailed', 'Condition failed (%s): %s', C.condition_id, ME.message);
...
if isempty(angParts)
    ...
    T_ang = vertcat(angParts{:});
end

%% Tables and summaries
writetable(T_patch, fullfile(OUT.table_dir, 'stage_c2_patch_level_predictions.csv'));
writetable(T_source, fullfile(OUT.table_dir, 'stage_c2_source_table.csv'));
writetable(T_log, fullfile(OUT.table_dir, 'stage_c2_condition_log.csv'));
writetable(T_ang, fullfile(OUT.table_dir, 'stage_c2_projected_angular_diagnostics.csv'));
write_all_summaries(T_patch, OUT);
write_homogeneous_oracle_summary(T_patch, OUT);
save(fullfile(OUT.data_dir, 'stage_C2_projected_diffuse3d_probe_results.mat'), 'T_patch','T_source','T_log','T_ang','CFG','-v7.3');
...
try
    ...
    warning('StageC2:ParallelPool', 'Could not start a parallel pool. Continuing serially: %s', ME.message);
end
end

function CFG = load_stage_config(root_dir)
configPath = fullfile(root_dir, 'configs', 'eikonal_validation', 'stage_c2_projected_diffuse3d_probe.json');
...
sig.REQ = CFG.REQ;
sig.PhaseModel = CFG.Simulation.PhaseModel;
if isfield(CFG.Simulation, 'Eikonal3D')
    sig.Eikonal3D = CFG.Simulation.Eikonal3D;
end
...
T = travel_time_for_source(phant, p, CFG);
...
function T = travel_time_for_source(phant, sourceXYZ, CFG)
phaseModel = lower(string(CFG.Simulation.PhaseModel));
opts = struct();
opts.ReferenceSpeed = median(phant.cs_map(:), 'omitnan');

switch phaseModel
    case "eikonal3d_projected"
        if max(phant.cs_map(:)) - min(phant.cs_map(:)) < 1e-12
            c0 = phant.cs_map(1);
            T = sqrt((phant.X - sourceXYZ(1)).^2 + ...
                     (CFG.Simulation.MeasurementPlaneY_m - sourceXYZ(2)).^2 + ...
                     (phant.Z - sourceXYZ(3)).^2) ./ c0;
            return;
        end
        y = projected_y_grid(CFG, sourceXYZ);
        e3 = CFG.Simulation.Eikonal3D;
        opts.MaxIterations = getfield_with_default(e3, 'MaxIterations', 80);
        opts.Tolerance = getfield_with_default(e3, 'Tolerance', 2e-6);
        opts.BoundaryInitialization = char(getfield_with_default(e3, 'BoundaryInitialization', 'all_faces_3d_time'));
        opts.NormalizeBoundaryTime = logical(getfield_with_default(e3, 'NormalizeBoundaryTime', false));
        opts.ReturnFullTravelTime = false;
        [T,~] = simcore.eikonal.fromPointSource3DExtruded( ...
            phant.cs_map, phant.x, y, phant.z, CFG.Simulation.MeasurementPlaneY_m, sourceXYZ, opts);
    case {"eikonal","eikonal2d","old_eikonal_2p5d_collapsed"}
        opts.MaxIterations = 220;
        opts.Tolerance = 1e-5;
        opts.NormalizeBoundaryTime = true;
        [T,~] = simcore.eikonal.fromPointSource2p5D(phant.cs_map, phant.x, phant.z, 0, sourceXYZ, opts);
    otherwise
        error('Unsupported Stage C2 phase model: %s', CFG.Simulation.PhaseModel);
end
end

function y = projected_y_grid(CFG, sourceXYZ)
e3 = CFG.Simulation.Eikonal3D;
dy = getfield_with_default(e3, 'dy_m', max(CFG.Simulation.dx_m, CFG.Simulation.dz_m));
margin = getfield_with_default(e3, 'YMarginM', 0.005);
planeY = CFG.Simulation.MeasurementPlaneY_m;
halfWidth = abs(sourceXYZ(2) - planeY) + margin;
halfWidth = max(halfWidth, dy);
N = ceil(halfWidth / dy);
y = planeY + (-N:N) * dy;
if ~any(abs(y - planeY) < 10*eps(max(1,abs(planeY))))
    y = sort([y planeY]);
end
end
...
if layoutFamily == "fibonacci"
    P = fibonacci_source_positions(N, Nin, L, CFG);
else
...
function P = fibonacci_source_positions(N, Nin, L, CFG)
% Build an approximately uniform 3-D source shell with a small controlled
% number of acquisition-plane sources. The implementation lives in simcore
% so later validation stages can reuse the same source geometry.
center = [CFG.Simulation.Lx_m/2, 0, CFG.Simulation.Lz_m/2];
if isfield(L, 'shell_radius_m')
    R = double(L.shell_radius_m);
else
    R = double(CFG.Simulation.FibonacciShellRadiusM);
end
src = simcore.sources.fibonacciSphereSources( ...
    'NumSources', N, ...
    'CenterXYZ', center, ...
    'ShellRadiusM', R, ...
    'NearPlaneCount', Nin, ...
    'PlaneY', CFG.Simulation.MeasurementPlaneY_m, ...
    'MotionAxis', CFG.Simulation.MotionDirection, ...
    'PhaseSeed', double(L.phase_seed), ...
    'Amplitude', 1.0, ...
    'MinOffPlaneYFraction', 0.18);
P = src.PositionsXYZ;
end
...
regime = string(C.field_regime);
inPlane = abs(S.positions_xyz(:,2)) < 1e-12;
assert(all(abs(S.motion_axes - repmat([0 0 1], size(S.motion_axes,1), 1)) < 1e-12, 'all'), ...
    'All Stage C2 source motion vectors must be [0 0 1].');
if regime == "single_source_lateral"
    assert(size(S.positions_xyz,1) == 1, 'single_source_lateral must have exactly one source.');
    assert(sum(inPlane) == 1, 'single_source_lateral must have one in-plane source.');
elseif startsWith(regime, "balanced_projected_diffuse_3D_")
    tok = regexp(char(regime), '(\d+)src', 'tokens', 'once');
    assert(~isempty(tok), 'Cannot parse balanced projected diffuse source count.');
    expectedN = str2double(tok{1});
    assert(size(S.positions_xyz,1) == expectedN, 'Balanced projected diffuse source count mismatch.');
    assert(sum(inPlane) >= 1, 'Balanced projected diffuse layout must have at least one in-plane source.');
    if expectedN == 64
        assert(sum(inPlane) >= 6 && sum(inPlane) <= 10, '64-source projected diffuse layout should have about 6-10 in-plane sources.');
    end
else
    tok = regexp(char(regime), 'diffuse_like_(\d+)src_layout([AB])', 'tokens', 'once');
    assert(~isempty(tok), 'Unknown Stage C2 field regime: %s', regime);
    expectedN = str2double(tok{1});
    layoutLetter = string(tok{2});
    if layoutLetter == "A"
        expectedIn = 1;
    else
        expectedIn = max(1, expectedN/4);
    end
    assert(size(S.positions_xyz,1) == expectedN, 'Diffuse-like source count mismatch.');
    assert(sum(inPlane) == expectedIn, 'Diffuse-like in-plane source count mismatch.');
end
end
...
for mi = 1:numel(modelNames)
    ...
    [qOracle, kTrue, swsOracle] = oracle_from_req_mapping(Faug.req_mapping, F.SWS_true, F.f0);
    R.q_oracle = qOracle;
    R.k_true = kTrue;
    R.SWS_oracle = swsOracle;
    R.q_error = q - qOracle;
    R.oracle_signed_error_pct = 100*(swsOracle - F.SWS_true) ./ F.SWS_true;
    R.oracle_abs_error_pct = abs(R.oracle_signed_error_pct);
...
function [qOracle, kTrue, swsOracle] = oracle_from_req_mapping(mappings, swsTrue, f0)
%ORACLE_FROM_REQ_MAPPING Return local REQ quantile matching true k.
% Evaluation-only diagnostic; never used as a frozen-model predictor.
n = numel(swsTrue);
qOracle = nan(n,1);
kTrue = 2*pi*f0(:) ./ swsTrue(:);
swsOracle = nan(n,1);
for i = 1:n
    qOracle(i) = k_to_quantile(mappings{i}, kTrue(i));
    swsOracle(i) = adaptive_req.quantile.quantile_to_cs(mappings{i}, qOracle(i), f0(i));
end
end

function q = k_to_quantile(mapping, kTarget)
if isempty(mapping) || ~isstruct(mapping) || ~isfield(mapping,'k_cent') || ~isfield(mapping,'Ecum') || ~isfinite(kTarget)
    q = NaN;
    return;
end
...
kClamped = min(max(kTarget, min(kuniq)), max(kuniq));
q = interp1(kuniq, Euniq, kClamped, 'linear', 'extrap');
q = min(max(q, 0), 1);
end

function write_homogeneous_oracle_summary(T, OUT)
%WRITE_HOMOGENEOUS_ORACLE_SUMMARY Compact q_pred-vs-q_oracle table for homogeneous cases.
if ~ismember("q_oracle", string(T.Properties.VariableNames))
    return;
end
H = T(startsWith(string(T.geometry), "homogeneous"), :);
if isempty(H)
    return;
end
G = findgroups(H.model_name, H.geometry, H.f0, H.field_regime, H.N_sources);
model_name = splitapply(@(x) x(1), H.model_name, G);
geometry = splitapply(@(x) x(1), H.geometry, G);
f0 = splitapply(@(x) x(1), H.f0, G);
field_regime = splitapply(@(x) x(1), H.field_regime, G);
N_sources = splitapply(@(x) x(1), H.N_sources, G);
N = splitapply(@numel, H.q_pred, G);
mean_q_pred = splitapply(@(x) mean(x,'omitnan'), H.q_pred, G);
mean_q_oracle = splitapply(@(x) mean(x,'omitnan'), H.q_oracle, G);
mean_q_error = splitapply(@(x) mean(x,'omitnan'), H.q_error, G);
median_q_error = splitapply(@(x) median(x,'omitnan'), H.q_error, G);
std_q_error = splitapply(@(x) std(x,'omitnan'), H.q_error, G);
MAPE_pct = splitapply(@(x) mean(x,'omitnan'), H.abs_error_pct, G);
bias_pct = splitapply(@(x) mean(x,'omitnan'), H.signed_error_pct, G);
oracle_MAPE_pct = splitapply(@(x) mean(x,'omitnan'), H.oracle_abs_error_pct, G);
oracle_bias_pct = splitapply(@(x) mean(x,'omitnan'), H.oracle_signed_error_pct, G);
S = table(model_name, geometry, f0, field_regime, N_sources, N, ...
    mean_q_pred, mean_q_oracle, mean_q_error, median_q_error, std_q_error, ...
    MAPE_pct, bias_pct, oracle_MAPE_pct, oracle_bias_pct);
writetable(S, fullfile(OUT.table_dir, 'stage_c2_homogeneous_q_oracle_summary.csv'));
end

function write_all_summaries(T, OUT)
writetable(summarize_metrics(T, "model_name"), fullfile(OUT.table_dir, 'stage_c2_summary_overall.csv'));
writetable(summarize_metrics(T, ["model_name","geometry"]), fullfile(OUT.table_dir, 'stage_c2_summary_by_geometry.csv'));
writetable(summarize_metrics(T, ["model_name","field_regime"]), fullfile(OUT.table_dir, 'stage_c2_summary_by_field_regime.csv'));
writetable(summarize_metrics(T, ["model_name","N_sources"]), fullfile(OUT.table_dir, 'stage_c2_summary_by_source_count.csv'));
writetable(summarize_metrics(T, ["model_name","N_in_plane_sources"]), fullfile(OUT.table_dir, 'stage_c2_summary_by_in_plane_source_count.csv'));
writetable(summarize_metrics(T, ["model_name","f0"]), fullfile(OUT.table_dir, 'stage_c2_summary_by_frequency.csv'));
writetable(summarize_metrics(T, ["model_name","geometry","field_regime"]), fullfile(OUT.table_dir, 'stage_c2_summary_by_geometry_field.csv'));
writetable(summarize_metrics(T, ["model_name","geometry","f0"]), fullfile(OUT.table_dir, 'stage_c2_summary_by_geometry_frequency.csv'));
writetable(summarize_metrics(T, ["model_name","roi_label"]), fullfile(OUT.table_dir, 'stage_c2_summary_by_roi.csv'));
writetable(summarize_metrics(T, ["model_name","roi_label","field_regime"]), fullfile(OUT.table_dir, 'stage_c2_summary_by_roi_field.csv'));
writetable(summarize_metrics(T, ["model_name","roi_label","N_sources"]), fullfile(OUT.table_dir, 'stage_c2_summary_by_roi_source_count.csv'));
...
writetable(W, fullfile(OUT.table_dir, 'stage_c2_worst_conditions.csv'));
end
...
function plot_representative_maps(sim, T, C, CFG, OUT)
Tc = T(string(T.model_name)=="q_spectrum_plus_composition",:);
swsMap = dense_map_from_table(Tc, Tc.SWS_pred, sim);
qMap = dense_map_from_table(Tc, Tc.q_pred, sim);
signErr = dense_map_from_table(Tc, Tc.signed_error_pct, sim);
absErr = dense_map_from_table(Tc, Tc.abs_error_pct, sim);
...
Dspec = projected_angular_diagnostics(sim, T(1,:), C, CFG, true);
ax = nexttile(tl); imagesc(ax, Dspec.kx, Dspec.kz, log10(Dspec.power2d + eps)); axis(ax,'image'); set(ax,'YDir','normal'); title(ax,'Local 2-D spectrum','FontSize',9); cb=colorbar(ax); ylabel(cb,'log power');
ax = nexttile(tl); plot(ax, Dspec.theta_centers, Dspec.angular_energy_norm, '-o', 'LineWidth', 1.1); grid(ax,'on'); title(ax, sprintf('Angular energy, H=%.2f', Dspec.angular_entropy),'FontSize',9); xlabel(ax,'angle (rad)'); ylabel(ax,'norm. energy');
...
fn = fullfile(folder, "stageC2_map__" + sanitize(C.condition_id) + ".png");
exportgraphics(fig, fn, 'Resolution', 180); close(fig);
end


function T = projected_angular_diagnostics(sim, T_condition, C, CFG, returnCurves)
if nargin < 5, returnCurves = false; end
D = local_projected_spectrum(sim.U, sim, CFG);
base = table(string(C.condition_id), string(C.geometry), double(C.f0), string(C.field_regime), double(C.M), ...
    D.angular_peak_to_mean, D.angular_cv, D.angular_entropy, D.radial_k50, D.radial_k90, ...
    'VariableNames', {'condition_id','geometry','f0','field_regime','M','angular_peak_to_mean','angular_cv','angular_entropy','radial_k50','radial_k90'});
if returnCurves
    T = D;
    return;
end
for ii = 1:numel(D.angular_energy_norm)
    base.(sprintf('Etheta_%02d', ii)) = D.angular_energy_norm(ii);
end
T = base;
end

function D = local_projected_spectrum(U, sim, CFG)
winMm = getfield_with_default(CFG.SpectrumDiagnostics, 'WindowSizeMm', 32);
nb = getfield_with_default(CFG.SpectrumDiagnostics, 'AngularBins', 36);
rb = getfield_with_default(CFG.SpectrumDiagnostics, 'RadialBins', 48);
lowFrac = getfield_with_default(CFG.SpectrumDiagnostics, 'LowKExcludeFraction', 0.06);
Nx = numel(sim.x); Nz = numel(sim.z);
cx = round(Nx/2); cz = round(Nz/2);
halfX = max(8, round((winMm*1e-3)/(2*sim.dx)));
halfZ = max(8, round((winMm*1e-3)/(2*sim.dz)));
ix = max(1,cx-halfX):min(Nx,cx+halfX);
iz = max(1,cz-halfZ):min(Nz,cz+halfZ);
W = real(U(iz,ix));
W = W - mean(W(:),'omitnan');
wx = hann_local(numel(ix)); wz = hann_local(numel(iz));
W = W .* (wz * wx');
P = abs(fftshift(fft2(W))).^2;
kx = fftshift((-floor(numel(ix)/2):ceil(numel(ix)/2)-1) / (numel(ix)*sim.dx));
kz = fftshift((-floor(numel(iz)/2):ceil(numel(iz)/2)-1) / (numel(iz)*sim.dz));
[KX,KZ] = meshgrid(kx,kz);
R = hypot(KX,KZ);
Theta = atan2(KZ,KX);
mask = R > lowFrac*max(R(:));
edges = linspace(-pi, pi, nb+1);
E = zeros(1,nb);
for b = 1:nb
    m = mask & Theta >= edges(b) & Theta < edges(b+1);
    E(b) = sum(P(m), 'omitnan');
end
if sum(E) <= eps
    En = ones(1,nb)/nb;
...
p = En / max(sum(En), eps);
ent = -sum(p .* log(max(p, eps))) / log(nb);
cv = std(E) / max(mean(E), eps);
pm = max(E) / max(mean(E), eps);
redges = linspace(0, max(R(:)), rb+1);
Er = zeros(1,rb);
for b = 1:rb
    m = R >= redges(b) & R < redges(b+1);
    Er(b) = sum(P(m), 'omitnan');
end
Ec = cumsum(Er) / max(sum(Er), eps);
rc = 0.5*(redges(1:end-1)+redges(2:end));
D = struct();
D.power2d = P;
D.kx = kx;
D.kz = kz;
D.theta_centers = 0.5*(edges(1:end-1)+edges(2:end));
D.angular_energy = E;
D.angular_energy_norm = En;
D.angular_peak_to_mean = pm;
D.angular_cv = cv;
D.angular_entropy = ent;
D.radial_centers = rc;
D.radial_energy = Er;
D.radial_cumulative = Ec;
D.radial_k50 = robust_cumulative_crossing(Ec, rc, 0.50);
D.radial_k90 = robust_cumulative_crossing(Ec, rc, 0.90);
end
...
function x = robust_cumulative_crossing(Ec, rc, target)
Ec = double(Ec(:)); rc = double(rc(:));
ok = isfinite(Ec) & isfinite(rc);
Ec = Ec(ok); rc = rc(ok);
if isempty(Ec)
    x = NaN;
    return;
end
[EcU, ia] = unique(Ec, 'stable');
rcU = rc(ia);
if numel(EcU) < 2 || target <= min(EcU)
    idx = find(Ec >= target, 1, 'first');
    if isempty(idx), idx = numel(rc); end
    x = rc(idx);
else
    x = interp1(EcU, rcU, target, 'linear', 'extrap');
end
end
...
function M = dense_map_from_table(T, vals, sim)
vals = double(vals);
if ismember('x_center_m', T.Properties.VariableNames)
    x = double(T.x_center_m);
    z = double(T.z_center_m);
elseif all(ismember({'ix','iz'}, T.Properties.VariableNames))
    x = sim.x(double(T.ix));
    z = sim.z(double(T.iz));
else
    M = nan(size(sim.cs_map));
    return;
end
ok = isfinite(x) & isfinite(z) & isfinite(vals);
[Xg,Zg] = meshgrid(sim.x, sim.z);
if nnz(ok) < 3
    M = nan(size(sim.cs_map));
    return;
end
try
    % Interpolate only inside the valid REQ-center hull. We intentionally do
    % not extrapolate outside valid windows, so non-estimable borders remain
    % blank instead of looking artificially reconstructed.
    F = scatteredInterpolant(x(ok), z(ok), vals(ok), 'natural', 'none');
    M = F(Xg, Zg);
catch
    F = scatteredInterpolant(x(ok), z(ok), vals(ok), 'nearest', 'none');
    M = F(Xg, Zg);
end
if ismember('x_center_m', T.Properties.VariableNames) && nnz(ok) >= 3
    try
        xok = x(ok);
        zok = z(ok);
        hullIdx = convhull(xok, zok);
        validHull = inpolygon(Xg, Zg, xok(hullIdx), zok(hullIdx));
        M(~validHull) = nan;
    catch
        % If the centers are degenerate, keep the non-extrapolating
        % scatteredInterpolant result.
    end
end
end

function plot_source_projection_panel(sim, C)
imagesc(sim.x*1e3, sim.z*1e3, sim.cs_map); axis image; set(gca,'YDir','normal'); hold on;
colormap(gca, parula); cb=colorbar; ylabel(cb,'SWS (m/s)');
P = C.source_positions;
if iscell(P), P = P{1}; end
if isnumeric(P) && size(P,2) == 3
    inPlane = abs(P(:,2)) < 1e-12;
    scatter(P(~inPlane,1)*1e3, P(~inPlane,3)*1e3, 30, 'k', 'filled');
    scatter(P(inPlane,1)*1e3, P(inPlane,3)*1e3, 50, 'r', 'filled');
end
title('Source projection','FontSize',10); xlabel('x (mm)'); ylabel('z (mm)');
end
...
fn = fullfile(folder, "stageC2_sources__" + sanitize(C.field_regime) + ".png");
exportgraphics(fig, fn, 'Resolution', 180);
savefig(fig, fullfile(folder, "stageC2_sources__" + sanitize(C.field_regime) + ".fig"));
close(fig);
end
...
imagesc(ax, M, 'AlphaData', isfinite(M)); axis(ax,'image'); set(ax,'YDir','normal');
...
MAPS.sws_pred = dense_map_from_table(Tmain, Tmain.SWS_pred, sim);
MAPS.q_pred = dense_map_from_table(Tmain, Tmain.q_pred, sim);
MAPS.signed_error_pct = dense_map_from_table(Tmain, Tmain.signed_error_pct, sim);
MAPS.abs_error_pct = dense_map_from_table(Tmain, Tmain.abs_error_pct, sim);
end
...
fprintf(fid, '# Stage C2: Projected Diffuse 3D Probe\n\n');
...
fprintf(fid, '- Phase model: `%s`.\n', string(CFG.Simulation.PhaseModel));
fprintf(fid, '- Homogeneous media use exact analytic 3D travel time in the projected mode; heterogeneous media use the extruded 3D Eikonal solver on the measurement plane.\n');
...
fprintf(fid, '## Outputs\n\nTables are under `tables/`; figures are under `figures/`. Representative maps include true SWS, amplitude, sign phase, real displacement, predicted SWS, signed error, absolute error, q, q-gradient, and source projection. Predicted maps are shown only over the valid REQ-center region; the script does not extrapolate SWS/q outside valid windows.\n\n');
fprintf(fid, '## Runtime\n\nObserved runner time for this run: %.1f minutes. Cached conditions are reused unless `ADAPTIVE_REQ_EIKONAL_STAGE_C2_FORCE_REBUILD=true`. Source-level parallelism used: %d. REQ-window parallelism used: %d.\n\n', elapsedSec/60, CFG.UseSourceParfor, CFG.UseWindowParfor);
...
fprintf(fid, 'Validation-only:\n\n```bash\nmatlab -batch "setenv(''ADAPTIVE_REQ_EIKONAL_STAGE_C2_MODE'',''validate_only''); run(''experiments/runners/eikonal_validation/run_stage_c2_projected_diffuse3d_probe.m'')"\n```\n\n');
fprintf(fid, 'Quick:\n\n```bash\nmatlab -batch "setenv(''ADAPTIVE_REQ_EIKONAL_STAGE_C2_MODE'',''quick''); run(''experiments/runners/eikonal_validation/run_stage_c2_projected_diffuse3d_probe.m'')"\n```\n\n');
fprintf(fid, 'Full and analysis:\n\n```bash\nmatlab -batch "setenv(''ADAPTIVE_REQ_EIKONAL_STAGE_C2_MODE'',''full''); run(''experiments/runners/eikonal_validation/run_stage_c2_projected_diffuse3d_probe.m'')"\nmatlab -batch "setenv(''ADAPTIVE_REQ_EIKONAL_STAGE_C2_MODE'',''full''); run(''experiments/analysis/eikonal_validation/analyze_stage_c2_projected_diffuse3d_probe.m'')"\n```\n');
fclose(fid);
end

function write_docs_readme(root_dir, CFG)
docDir = fullfile(root_dir, 'docs', 'eikonal_validation'); ensure_dir(docDir);
fid = fopen(fullfile(docDir, 'stage_C2_projected_diffuse3d_probe.md'), 'w'); assert(fid>0);
fprintf(fid, '# Stage C2: Projected Diffuse 3D Probe\n\n');
fprintf(fid, 'Stage C2 tests whether Fibonacci-shell projected 3D diffuse fields produce a more isotropic x-z spectrum than the previous Stage C diffuse-like layouts. It uses no noise, no correction, and no estimability mask.\n\n');
fprintf(fid, 'The runner uses the `%s` phase model. In homogeneous media this is evaluated with the exact analytic 3D travel time; in heterogeneous media it uses the extruded 3D Eikonal solver. Representative predicted maps are intentionally blank outside valid REQ windows rather than extrapolated.\n\n', string(CFG.Simulation.PhaseModel));
